...
def get_enforcement_priorities(limit: int = 10) -> dict[str, Any]:
    ...
    for i, zone_id in enumerate(VALID_ZONES):
        try:
            ...
            # PRIMARY: use live attribution AQI (real-time OWM measurement)
            # This is always season-correct and reflects current pollution levels.
            # Forecast values reflect seasonal SARIMA patterns (e.g. high winter AQI)
            # which would show inflated values in summer — wrong for enforcement.
            aqi = attr["currentAQI"]

            # Only fall back to forecast if live AQI is missing/zero
            if not aqi:
                try:
...
def build_reason_text(zone_id: str, aqi: int, population: int, dominant_source: str, confidence: float) -> str:
    ...
    actions = {
        ...
        "biomass_burning": "Alert local enforcement to patrol for open waste or biomass burning in the periphery. Elevated CO and PM2.5 indicate low-temperature combustion."
    }
